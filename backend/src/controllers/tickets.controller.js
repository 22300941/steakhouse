...
const getTicketById = (req, res) => {
  ...
  db.query('SELECT * FROM tickets WHERE id = ?', [id], (err, rows) => {
    ...
    db.query('SELECT * FROM ticket_items WHERE ticket_id = ?', [id], (err2, items) => {
      if (err2) return res.status(500).json({ error: 'Error al obtener items del ticket.' });
      ticket.items = items;
      res.json(ticket);
    });
  });
...
const crearTicket = (req, res) => {
  const { fecha, responsable, cantidad, items } = req.body;
  const sql = 'INSERT INTO tickets (fecha, responsable, cantidad) VALUES (?, ?, ?)';

  db.query(sql, [fecha, responsable, cantidad], (err, result) => {
    ...
    if (items && items.length > 0) {
      ...
      db.query(itemsSql, [valores], (err2) => {
        if (err2) return res.status(500).json({ error: 'Error al guardar items del ticket.' });
        res.status(201).json({ id: ticketId, fecha, responsable, cantidad });
      });
    } else {
      res.status(201).json({ id: ticketId, fecha, responsable, cantidad });
    }
